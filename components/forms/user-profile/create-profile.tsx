...
export const CreateProfileOne: React.FC<ProfileFormType> = ({
  ...
  const steps = [
    ...
    {
      id: "Step 2",
      name: "Carpool Vehicle Information",
      // fields are mapping and flattening for the error to be trigger  for the dynamic fields
      fields: fields
        ?.map((_, index) => [
          `vehicles.${index}.makeAndModel`,
          `vehicles.${index}.licensePlateNumber`,
          `vehicles.${index}.vehicleYear`,
          `vehicles.${index}.vehicleCondition`,
          `vehicles.${index}.seatsAvailable`,
  ...
  const countries = [{ id: "in", name: "India" }];
  const cities = [{ id: "2", name: "Vijayawada" }];
        ...
        <ul className="flex gap-4">
          {steps.map((step, index) => (
            <li key={step.name} className="md:flex-1">
              {currentStep > index ? (
                <div className="group flex w-full flex-col border-l-4 border-blue-700 py-2 pl-4 transition-colors md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4">
                  <span className="text-sm font-medium text-blue-700 transition-colors ">
                    {step.id}
                  </span>
                ...
                <div
                  className="flex w-full flex-col border-l-4 border-blue-700 py-2 pl-4 md:border-l-0 md:border-t-4 md:pb-0 md:pl-0 md:pt-4"
                  aria-current="step"
                >
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          disabled={loading}
                          placeholder="joegatto@gmail.com"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
                    ...
                    <AccordionItem value="item-1">
                      <AccordionTrigger
                        className={cn(
                          ...
                          errors?.vehicles?.[index] && "text-red-700",
                        )}
                      >
                        {`Vehicle ${index + 1}`}
                      ...
                      <AccordionContent>
                        ...
                        >
                          ...
                          <FormField
                            control={form.control}
                            name={`vehicles.${index}.makeAndModel`}
                            render={({ field }) => (
                              <FormItem>
                          ...
                          <FormField
                            control={form.control}
                            name={`vehicles.${index}.seatsAvailable`}
                            render={({ field }) => (
                              <FormItem>
                                ...
                                >
                                  <FormControl>
                                    <SelectTrigger>
                                      <SelectValue
                                        defaultValue={field.value}
                                        placeholder="Select the number of seats available"
                                      />
                                    </SelectTrigger>
                                  </FormControl>
                                  <SelectContent>
                          ...
                          <FormField
                            control={form.control}
                            name={`vehicles.${index}.vehicleCondition`}
                            render={({ field }) => (
                              <FormItem>
